...
ALTER TABLE services ADD COLUMN IF NOT EXISTS response_time INTEGER;

-- Create index on response_time for performance queries
CREATE INDEX IF NOT EXISTS idx_services_response_time ON services(response_time) WHERE response_time IS NOT NULL;
